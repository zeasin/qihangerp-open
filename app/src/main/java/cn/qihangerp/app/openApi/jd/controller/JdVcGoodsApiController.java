...
import cn.qihangerp.sdk.jd.VcGoodsApiHelper;
import cn.qihangerp.open.jd.domain.JdVcGoods;
import cn.qihangerp.sdk.jd.response.JdVcGoodsResponse;
import cn.qihangerp.open.jd.service.JdVcGoodsService;
...
@RestController
@AllArgsConstructor
public class JdVcGoodsApiController {
    private final JdApiCommon jdApiCommon;
    private final JdVcGoodsService jdVcGoodsService;
...
//        String pullParams = "{PageNo:1,PageSize:50,startTime:"+startTime+",endTime:"+endTime+"}";
        ...
        ApiResultVo<JdVcGoodsResponse> resultVo = VcGoodsApiHelper.pullGoodsList(serverUrl, appKey, appSecret, accessToken, startTime, endTime);
        if(resultVo.getCode() !=0 ){
            OShopPullLogs logs = new OShopPullLogs();
            logs.setShopId(params.getShopId());
            logs.setShopType(EnumShopType.JDVC.getIndex());
            logs.setPullType("GOODS");
            logs.setPullWay("主动拉取商品");
        ...
        for (var goods: resultVo.getList()){
            JdVcGoods jdGoodsSku = new JdVcGoods();
            BeanUtils.copyProperties(goods,jdGoodsSku);
            jdVcGoodsService.saveGoods(params.getShopId(),jdGoodsSku);
            successTotal++;
        }
        // 添加拉取日志
        OShopPullLogs logs = new OShopPullLogs();
